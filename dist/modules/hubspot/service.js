...
class HubspotService {
    ...
    async sincronizarContactos() {
        ...
        do {
            ...
            const responseContactos = response.results.map((c) => ({
                id: c.id,
                ...
                provincia_de_procedencia: c.properties.provincia_de_procedencia ?? null,
                ...
                fecha_de_inicio_academico: c.properties.fecha_de_inicio_academico ?? null,
    ...
    async ejecutarSincronizacionCompleta() {
        ...
        try {
            ...
            if (!gotLock) {
                ...
                const db = this.repo.db("API_2");
                const enCurso = await db("api_hubspot_sync_log")
                ...
                return {
                    ...
                    elapsedSeconds: enCurso?.startedAt
                        ? Math.floor((Date.now() - new Date(enCurso.startedAt).getTime()) / 1000)
                        : null,
                };
            }
