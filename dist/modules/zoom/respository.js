...
class ZoomRepository {
    ...
    async getCourseIdsByShortnames(shortnames) {
    ...
    async getAttendanceConfig() {
        const [rows] = await this.db("API_2").raw(`SELECT * FROM zoom_attendance_config LIMIT 1`);
        return rows[0];
    }
    async getInstances(filters) {
        const db = this.db("API_2");
        const query = db("zoom_meeting_instance")
            .join("zoom_meeting", "zoom_meeting_instance.meeting_id", "zoom_meeting.id")
            .select("zoom_meeting_instance.*", "zoom_meeting.shortname", "zoom_meeting.courseid");
    ...
    async getDocentes(courseid, n_numdia) {
        const [row] = await this.db("SIGU_LECTURA").raw(`
        SELECT 
          DISTINCT
            a.c_dnidoc,
    ...
    async getDocenteParticipantes(instance_id) {
        const [row] = await this.db("API_2").raw(`
    ...
    async sesionExistente(courseid, d_fecha, c_dnidoc) {
        const [row] = await this.db("SIGU_LECTURA").raw(`
      SELECT 
          a.*
      FROM
    ...
    async insertZoomMeetingParticipantRaw(participants) {
    ...
    async insertZoomMeetingParticipants(participants) {
        if (!participants.length)
            return;
        const db = this.db("API_2");
        const now = new Date();
        const rows = participants.map((p) => ({
            instance_id: p.instance_id ?? null,
            zoom_user_id: p.zoomUser_id && p.zoomUser_id.trim() !== "" ? p.zoomUser_id : null,
            name: p.name ?? null,
            email: p.email ?? null,
            role: p.role ?? null,
    ...
    async upsertZoomRoom(zoomRooms) {
        if (!zoomRooms.length)
            return true;
        const db = this.db("API_2");
        const now = new Date();
    ...
    async upsertZoomMeetings(meetings) {
        if (!meetings.length)
            return;
        const db = this.db("API_2");
        const now = new Date();
    ...
    async upsertZoomMeetingOccurrences(occurrences) {
    ...
    async upsertZoomMeetingInstances(instances) {
        if (!instances.length)
            return;
        const db = this.db("API_2");
        const now = new Date();
        ...
        const has = (key) => instances.some((i) => i[key] !== undefined);
        if (has("meeting_id")) {
    ...
    async getMatriculadosCantindad(courseid, d_date) {
        const [row] = await this.db("SIGU_LECTURA").raw(`
        SELECT
          COUNT(*) as cantidad
        FROM
    ...
    async getHorarioGrupo(courseid, n_numdia, c_dnidoc) {
        const params = [courseid, n_numdia];
        let docenteWhere = "";
        if (c_dnidoc?.trim()) {
    ...
    async getSesiones(n_codper, courseid, d_fecha, c_dnidoc) {
        const [row] = await this.db("SIGU_LECTURA").raw(`
      SELECT 
          h.*
      FROM 
    ...
    async createAsistenciaDetalles(data) {
        if (!data.length) {
            return {
                affectedRows: 0,
            };
