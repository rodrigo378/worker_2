...
import { UpsertZoomUserInput, Zoom_User } from "./types/db.types";
...
export class ZoomRepository {
  ...
  // async getCourseid(shortname: string) {
  ...
  // async getInstances() {
  //   const [rows] = await this.db("API_2").raw(`
  //     SELECT * FROM zoom_meeting_instances LIMIT 1
  //   `);
  ...
  // async upsertZoomMeetings(meetings: Partial<ZoomMeeting>[]) {
  ...
  // async upsertZoomMeetingInstances(instances: Partial<ZoomMeetingInstance>[]) {
  ...
  // async upsertZoomMeetingDetails(details: Partial<ZoomMeetingDetail>[]) {
  ...
  // async createZoomSyncLog(log: Partial<ZoomSyncLog>) {
  //   const db = this.db("API_2");
  //   const now = new Date();
